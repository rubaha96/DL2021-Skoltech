...
class FloodNet(Dataset):
    ...
    def __init__(
        ...
        if augment:
            ...
            A.RandomScale(), # Random resize
            A.RandomCrop(width=self.img_size, height=self.img_size), # Random crop (within image borders, output size = img_size)
            A.Rotate(), # Random rotation
            A.HorizontalFlip(),
            A.VerticalFlip(), # Random horizontal and vertical Flip
            A.RandomBrightness(), # Random color augmentation
            ])
            # self.transform = None
